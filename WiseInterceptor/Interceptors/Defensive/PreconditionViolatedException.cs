...
namespace WiseInterceptor.Interceptors.Defensive
{
    ...
    {
        public BlockDefaultValuePreconditionViolatedException(IInvocation invocation, IUtils utils)
            : base(GetExceptionDescription(invocation, utils))
        {
        }

        private static string GetExceptionDescription(IInvocation invocation, IUtils utils)
        {
            StringBuilder message = new StringBuilder();
            for (int i = 0; i < invocation.Arguments.Count(); i++)
            {
                if (invocation.Arguments[i].Equals(utils.GetDefaultValue(invocation.Arguments[i].GetType())))
                {
